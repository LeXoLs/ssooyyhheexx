...
exports.run = (client, message, args) => {
 message.delete();
    message.guild.createChannel(`talep-${message.author.username}`, 'text').then(ch => {
        ...
        .setAuthor("»  SoyHex | Destek Sistemi")
        .setDescription("**Yetkili Arkadaşlar Sizinle İlgilenecektir.\nDestek Talebini İptal Etmek İçin \`[s!kapat]\` Yazabilirsin!**")
        .setFooter('  SoyHex | Destek Sistemi', client.user.avatarURL)
        .setTimestamp()
        ch.send(embed).catch()
        ch.send(`${message.author.username} `)
        ch.awaitMessages((msg)=> {
            if (msg.content === "s!kapat") {
